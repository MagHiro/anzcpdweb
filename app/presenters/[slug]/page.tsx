...
import { featuredCourses, getPresenter } from "@/lib/domain/cpd";
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  let presenter = getPresenter(slug);
  try { presenter = await getPresenterBySlug(slug) ?? presenter; } catch { /* Use the static brochure profile when the database is unavailable. */ }
...
export default async function PresenterPage({ params }: { params: Params }) {
  const { slug } = await params;
  const fallbackPresenter = getPresenter(slug);
  let databaseAvailable = true;
  let presenter = null as Awaited<ReturnType<typeof getPresenterBySlug>> | typeof fallbackPresenter;
  try { presenter = await getPresenterBySlug(slug); } catch { databaseAvailable = false; presenter = fallbackPresenter; }
  if (!presenter) notFound();
  const classes = databaseAvailable ? await getPublishedClasses({ presenterSlug: presenter.slug, time: "upcoming", pageSize: 12 }).catch(() => []) : [];
  const courses = featuredCourses.filter((course) => course.presenterSlug === presenter.slug);
  ...
  return <>
    ...
    <Section><Container><div className="grid gap-12 lg:grid-cols-[.62fr_1.38fr]"><aside><p className="eyebrow text-[var(--fern)]">Areas of focus</p><div className="mt-5 flex flex-wrap gap-2 lg:grid">{presenter.expertise.map((item) => <span key={item} className="rounded-full bg-[var(--mist)] px-4 py-2 text-xs font-bold text-[var(--forest)] lg:rounded-xl">{item}</span>)}</div><LinkButton href="/courses" variant="secondary" className="mt-8">Browse all activities <ArrowRight className="ml-2" size={16} /></LinkButton></aside><div><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow text-[var(--fern)]">Led by {presenter.name}</p><h2 className="display mt-3 text-4xl text-[var(--forest)]">Current activities</h2></div><Link href="/presenters" className="focus-ring text-sm font-bold text-[var(--forest)] underline decoration-[#e87954] underline-offset-4">All presenters</Link></div>{classes.length ? <div className="mt-8 grid gap-5 md:grid-cols-2">{classes.map((item) => <ClassCard key={item.id} item={item} />)}</div> : <div className="mt-8 grid gap-5 md:grid-cols-2">{courses.map((course) => <CourseCard key={course.slug} course={course} />)}</div>}</div></div></Container></Section>
  </>;
}
