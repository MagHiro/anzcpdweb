...
const categoryTones = ["bg-[#fff4ef] text-[#a7472b]", "bg-[#f1f7fb] text-[#2d617d]", "bg-[#fff8e9] text-[#97691a]", "bg-[#eef7f1] text-[#21604b]"];
...
export default async function HomePage() {
  ...
  return <>
      ...
      <section className="relative overflow-hidden bg-[var(--forest)] text-white">
        ...
        <Container className="relative grid min-h-[650px] items-center gap-14 py-20 lg:grid-cols-[1.08fr_.92fr] lg:py-24">
          <div className="max-w-3xl">
            ...
            <div data-hero-meta className="mt-12 flex flex-wrap gap-x-8 gap-y-4 border-t border-[#4d7468] pt-5 text-xs font-semibold text-[#b9d6c7]"><span><strong className="text-white">{activityTypes.length || "—"}</strong> catalogue formats</span><span><strong className="text-white">{countries.length || "—"}</strong> professional contexts</span><span><strong className="text-white">Secure</strong> online registration</span></div>
          </div>
    ...
    <Section className="bg-[#fbfaf6]"><Container><div className="grid gap-10 lg:grid-cols-[.68fr_1.32fr]"><Reveal><p className="eyebrow text-[var(--fern)]">Browse the catalogue</p><h2 className="display mt-4 max-w-md text-4xl leading-[1.02] text-[var(--forest)] sm:text-5xl">CPD that fits the work.</h2><p className="mt-5 max-w-sm text-base leading-7 text-[var(--muted)]">Start with a subject from the live catalogue, then compare the format, presenter, time commitment and CPD value recorded for each class.</p><Link href="/cpd-requirements" className="focus-ring mt-7 inline-flex items-center gap-2 text-sm font-bold text-[var(--forest)] underline decoration-[#e87954] underline-offset-4">Read the catalogue guide <ArrowUpRight size={16} /></Link></Reveal><div className="grid gap-3 sm:grid-cols-2">{categories.slice(0, 4).map((category, index) => <Reveal key={category.id}><Link href={`/courses?country=${category.country}&category=${encodeURIComponent(category.slug)}`} className="group block rounded-[1.25rem] border border-[var(--line)] bg-white p-5 transition hover:-translate-y-1 hover:border-[#a9c7bd] hover:shadow-[0_14px_38px_rgba(23,60,55,.07)]"><div className="flex items-start justify-between gap-3"><span className={`flex h-9 w-9 items-center justify-center rounded-full text-[11px] font-black ${categoryTones[index % categoryTones.length]}`}>{String(index + 1).padStart(2, "0")}</span><span className="text-[10px] font-extrabold uppercase tracking-[.12em] text-[var(--muted)]">{countryNames.get(category.country) ?? category.country}</span></div><h3 className="mt-7 text-lg font-bold text-[var(--forest)]">{category.name}</h3><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{category.shortDescription ?? "Explore published CPD classes in this catalogue category."}</p><span className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-[var(--forest)] group-hover:text-[var(--fern)]">View classes <ArrowRight size={14} /></span></Link></Reveal>)}{!categories.length ? <p className="rounded-[1.25rem] border border-dashed border-[var(--line)] bg-white p-6 text-sm leading-6 text-[var(--muted)] sm:col-span-2">Catalogue categories will appear when the database has published taxonomy records.</p> : null}</div></div></Container></Section>

    <Section className="bg-white"><Container><div className="flex flex-wrap items-end justify-between gap-6"><div><p className="eyebrow text-[var(--fern)]">The next dates</p><h2 className="display mt-3 text-4xl text-[var(--forest)] sm:text-5xl">Find your next activity.</h2></div><LinkButton href="/courses" variant="quiet">View all activities <ArrowRight className="ml-2" size={16} /></LinkButton></div>{liveClasses.length ? <div className="mt-10 grid gap-5 lg:grid-cols-3">{liveClasses.map((item) => <Reveal key={item.id}><ClassCard item={item} /></Reveal>)}</div> : <p className="mt-10 rounded-[1.25rem] border border-dashed border-[var(--line)] p-8 text-center text-sm text-[var(--muted)]">No upcoming published classes are available yet.</p>}</Container></Section>
    ...
    <Section className="bg-[var(--forest)] text-white"><Container><div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="eyebrow text-[#a9cfc0]">Need a clearer plan?</p><h2 className="display mt-4 max-w-3xl text-4xl leading-[1.02] text-white sm:text-6xl">Start with the catalogue guide.</h2><p className="mt-5 max-w-xl text-base leading-7 text-[#c5d8ce]">Review the live categories and activity formats maintained in the database, then choose the next useful piece of learning for your practice.</p></div><LinkButton href="/cpd-requirements" className="bg-[#e87954] text-white hover:bg-[#f28c68]">View catalogue guide <ArrowUpRight className="ml-2" size={17} /></LinkButton></div></Container></Section>
  </>;
}
