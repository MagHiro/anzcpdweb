...
const navigation = [
  ...
  { href: "/cpd-requirements", label: "The guide" },
...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b border-[var(--line)]/80 bg-[var(--surface)]/95 backdrop-blur-xl">
    <div className="hidden border-b border-[var(--forest)]/20 bg-[var(--forest)] text-[#d9e8df] sm:block">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-2 text-[10px] font-bold uppercase tracking-[.12em] sm:px-8 lg:px-12">
        <p>AU &amp; NZ Migration Academy <span className="mx-2 text-[var(--signal)]">•</span> CPD for migration professionals</p>
    ...
    <div className="mx-auto flex min-h-[78px] max-w-[1440px] items-center justify-between gap-6 px-5 sm:px-8 lg:px-12">
      <Link href="/" className="focus-ring group flex items-center gap-3" onClick={() => setOpen(false)}>
        <span className="relative flex h-11 w-11 items-center justify-center rounded-full border border-[var(--forest)] text-[13px] font-black tracking-[-.1em] text-[var(--forest)] transition group-hover:bg-[var(--forest)] group-hover:text-white"><span>ANZ</span><span className="absolute -bottom-1 -right-1 h-2.5 w-2.5 rounded-full bg-[var(--signal)]" /></span>
        <span className="leading-none"><span className="block text-[14px] font-extrabold tracking-[.1em] text-[var(--forest)]">ANZ MIGRATION</span><span className="mt-1 block text-[9px] font-bold uppercase tracking-[.24em] text-[var(--muted)]">Academy · CPD</span></span>
      </Link>
      ...
      <nav id="site-navigation" className={cn("items-center gap-2 text-sm font-semibold text-[var(--muted)] md:flex", open ? "absolute inset-x-4 top-[calc(100%+1px)] flex flex-col items-stretch gap-1 rounded-b-2xl border border-t-0 border-[var(--line)] bg-[var(--surface)] p-4 shadow-[0_22px_55px_rgba(18,36,31,.12)] md:static md:flex-row md:items-center md:border-0 md:bg-transparent md:p-0 md:shadow-none" : "hidden")}>
        {navigation.map((item) => <Link key={item.href} className={cn("focus-ring rounded-lg px-3 py-3 hover:text-[var(--forest)] md:py-2", isActive(item.href) && "bg-[var(--mist)] text-[var(--forest)] md:bg-transparent md:underline md:decoration-[var(--signal)] md:underline-offset-8")} href={item.href} onClick={() => setOpen(false)}>{item.label}</Link>)}
        <Link className="focus-ring mt-2 inline-flex items-center justify-center gap-2 rounded-full bg-[var(--forest)] px-5 py-3 text-center text-white hover:bg-[var(--fern)] md:ml-3 md:mt-0 md:py-2.5" href="/register" onClick={() => setOpen(false)}>Account <ArrowUpRight size={15} /></Link>
      </nav>
    </div>
  </header>;
}
