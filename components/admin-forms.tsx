...
function ArchiveCategoryButton() { const [state, action, pending] = useAdminAction(archiveCategoryAction); return <><button disabled={pending} type="submit" formAction={action} onClick={(event) => { if (!window.confirm("Archive this category? Existing classes and historical bookings will remain intact.")) event.preventDefault(); }} className="focus-ring rounded-full border border-[#e8bbb6] px-4 py-2.5 text-sm font-bold text-[var(--error)]">{pending ? "…" : "Archive"}</button>{state.message ? <span className="sr-only" role="status">{state.message}</span> : null}</>; }

function ArchivePresenterButton() { const [state, action, pending] = useAdminAction(archivePresenterAction); return <><button disabled={pending} type="submit" formAction={action} onClick={(event) => { if (!window.confirm("Archive this presenter? Existing class assignments will remain intact.")) event.preventDefault(); }} className="focus-ring rounded-full border border-[#e8bbb6] px-4 py-2.5 text-sm font-bold text-[var(--error)]">{pending ? "…" : "Archive"}</button>{state.message ? <span className="sr-only" role="status">{state.message}</span> : null}</>; }
...
export function AdminClassForm({ classRecord, categories, presenters: presenterOptions = [], onSaved }: { classRecord?: ClassRecord; categories: CategoryRecord[]; presenters?: PresenterRecord[]; onSaved?: () => void }) {
  ...
  return <form action={action} className="space-y-6"><input type="hidden" name="id" value={classRecord?.id ?? ""} /><FormNotice className="mb-1">{state.message}</FormNotice>
    <section className="rounded-[1.2rem] border border-[var(--line)] bg-white p-5 sm:p-6"><p className="eyebrow text-[var(--fern)]">Basic information</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <section className="rounded-[1.2rem] border border-[var(--line)] bg-white p-5 sm:p-6"><p className="eyebrow text-[var(--fern)]">Schedule and delivery</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <section className="rounded-[1.2rem] border border-[var(--line)] bg-white p-5 sm:p-6"><p className="eyebrow text-[var(--fern)]">Capacity and CPD</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <section className="rounded-[1.2rem] border border-[var(--line)] bg-white p-5 sm:p-6"><p className="eyebrow text-[var(--fern)]">Pricing and booking window</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
      <Field label="Price in minor units" hint="AUD 195.00 = 19500." required><TextInput name="priceMinorUnits" type="number" min={0} step={1} defaultValue={classRecord?.priceMinorUnits ?? 0} required /></Field>
    ...
    <section className="rounded-[1.2rem] border border-[var(--line)] bg-white p-5 sm:p-6"><p className="eyebrow text-[var(--fern)]">Media and search</p><div className="mt-5 grid gap-5 sm:grid-cols-2">
...
function ActionForm({ action, id, label, variant = "primary", confirmMessage }: { action: (formData: FormData) => Promise<AdminActionState>; id: string; label: string; variant?: "primary" | "secondary" | "danger"; confirmMessage?: string }) { const [state, formAction, pending] = useAdminAction(action); return <form action={formAction} onSubmit={(event) => { if (confirmMessage && !window.confirm(confirmMessage)) event.preventDefault(); }}><input type="hidden" name="id" value={id} /><button type="submit" disabled={pending} className={`focus-ring min-h-10 rounded-full px-4 text-xs font-bold ${variant === "primary" ? "bg-[var(--forest)] text-white" : variant === "danger" ? "border border-[#e8bbb6] text-[var(--error)]" : "border border-[var(--line)] text-[var(--forest)]"}`}>{pending ? "…" : label}</button>{state.message ? <span className="sr-only" role="status">{state.message}</span> : null}</form>; }
